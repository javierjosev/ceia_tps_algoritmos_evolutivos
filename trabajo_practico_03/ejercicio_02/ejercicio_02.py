...
def plot_vector(vector, num_part):
    ...
    if vector is not None:
        ...
        ax.plot(
            x_vector, vector, marker="o", linestyle="-", color="b", label="Iteración"
        )
...
variables = ["a", "b"]
inequalities = ["300*a + 400*b <= 127000", "20*a + 10*b <= 4270", "a >= 0", "b >= 0"]
restricted_pso = RestrictedPSO()
gbest, gbest_fit, gbest_fit_hist = restricted_pso.run(
    n_particles, n_dimensions, max_iterations, f, w, c1, c2, inequalities, variables
)
