...
class GeneticAlgorithm:
    ...
    def __init__(self, expression, num_individuals, bits_x, num_generations, prob_crossover, prob_mutation,
                 x_min, x_max):
        ...
        self.population = [''.join(random.choices(['0', '1'], k=bits_x)) for _ in range(num_individuals)]
    ...
    def _evaluate_population(self, population):
        ...
        for chromosome in population:
            x = self._decode_chromosome(chromosome)
            fitness.append(self._fitness_function(x=x))
    ...
    def _crossover(self, parent1, parent2):
        if random.random() < self.prob_crossover:
            point = random.randint(1, self.bits_x - 1)
    ...
    def run(self):
        ...
        best_x = self._decode_chromosome(best_chromosome)
        best_fitness = fitness[best_index]

        return best_chromosome, best_x, best_fitness, self.best_fitness_value_per_gen
